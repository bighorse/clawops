...
3. **同一个人在 Web 与小程序是两份独立实例。** Web 用 `web:<user_id>`，小程序用微信 `openid`，两者是不同的键——同一个人从两端登录，会得到**两套互不相通的对话记忆和简报库**。如果需要打通，得由你们建立"微信 openid ↔ user_id"的映射，两端登录时统一报同一个标识。**这一条建议在开发前就定下来**，上线后再合并历史数据会很麻烦。
...
| Web/小程序身份是否打通 | ⬜ **建议开发前定**（§3.8.5 第 3 条），上线后合并历史很麻烦 |
| 公网开放 | ⬜ 待上一项就绪后由运维放开 |
| 企微接入 | ⬜ 接口已就绪，待后续实现 |
