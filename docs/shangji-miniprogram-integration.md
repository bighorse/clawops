...
| Web 端跨域白名单 | ✅ 已支持（含 localhost:8080 / 5173）；**生产域名待你们提供**（§3.8.1） |
| Web 端登录 `/auth/web-login` | ✅ 已上线（§3.8.5），待你们后端接入 |
| Web/小程序身份 | ✅ 已定：**两端各自独立**，不做映射（§3.8.5 第 3 条） |
| 公网开放 | ✅ 已开放，可直接联调 |
| 企微接入 | ⬜ 接口已就绪，待后续实现 |
