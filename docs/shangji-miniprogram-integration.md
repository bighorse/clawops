...
- **小程序登录（`/auth/wx-login`）当前不可用**。它依赖 §4 的 code2session 接口，在你们实现之前会返回 400。**Web 端不受影响**，走 `/auth/web-login`（§3.8.5）。
- **`/internal/*` 一律 404**。那是服务端内部 webhook，对外封闭，不在对接范围内。
...
| 400 | `{"error":"今日额度已用完，请稍后再试或联系管理员调整额度上限。"}` | **每用户每日额度耗尽**，详见 §3.9 | 原样提示用户，不要当成系统故障 |
| 500 | `{"error":"..."}` | 服务端错误。文案里会带上游的具体原因，便于排障 | 提示稍后再试 |

统一错误处理**必须兼容 body 不是 JSON 的情况**，否则 401 时前端会解析崩溃。

另外：**接口本身从不返回 403**。若收到 403，说明请求被网络层的访问控制挡了，联系运维确认。
...
#### 3.6.1 ⚠️ 先理解这个通道是什么：它不是"流式回答"

**`/events` 推的是执行过程，不是回答内容。** 这一点常被误解，请先看清楚：

| | 走哪里 |
|---|---|
| **回答正文** | `POST /chat` 的 **HTTP 响应**（一次性返回完整文本，不分块） |
| **执行过程**（调了什么工具、耗时多久） | `GET /events` 事件流 |
| **长任务状态**（企业快评跑到哪了） | `GET /events` 的 `sop_task` 事件 |

所以**不要按 OpenAI 那种流式接口的思路来用它**——这里没有逐 token 推送，也**没有 `data: [DONE]` 之类的结束标记**，等它会永远等不到。

各种"结束"该怎么判断：

| 场景 | 结束信号 |
|---|---|
| 普通问答 | **`/chat` 的 HTTP 响应返回** —— 就这一个，事件流里不会再有任何标记 |
| 企业快评（长任务） | 事件流里的 `sop_task` 且 `event: "done"` |

**一次普通问答在事件流里通常只有一条 `llm_request`**，然后就没了——不是断了，是本来就这样。调了工具才会有 `tool_call_start` / `tool_call`。

##### 心跳与空行

服务端**每 15 秒发一次心跳**，格式是 SSE 的注释行：
...
即一个冒号加空行。**解析时必须跳过它**——只处理 `data:` 开头的行，否则拿去 `JSON.parse` 会报错：

```js
if (!line.startsWith('data:')) continue   // 跳过心跳与空行
```

调试工具（Apifox、Postman 等）会把心跳显示成一行孤零零的 `:`，**看到它说明连接是活的**，不是出错。

#### 3.6.2 ⚠️ 长任务是异步的，`/chat` 会先返回

这是实现滚动显示时**最容易踩的坑**：

- **普通问答**是同步的 —— `/chat` 一直等到答案出来才返回，期间事件流推工具调用。
- **企业快评是异步的** —— 服务端一旦发起流程就**立刻**返回"已为您发起「企业快评」"，`/chat` 请求随即结束。**但流程还要再跑 3–5 分钟**，事件流会持续推送这段时间的每一步。

如果你的展示条件写成「请求进行中才显示执行过程」，那么**企业快评发起后的那几分钟里，界面上什么都不会有** —— 而那恰恰是用户最需要反馈的时段。

正确做法是同时看两个状态：

```js
// 请求进行中（同步问答） 或 长任务后台运行中（异步流程）
const showLiveSteps = this.busy || this.sopRunning
```

`sopRunning` 由 `sop_task` 事件驱动：`created` / `running` 置真，`done` 置假并清空步骤。

**另外**：用户刷新页面时，长任务可能已经在跑了 —— `created` 事件早已发过，光靠事件流补不回状态。所以**页面挂载时要主动查一次 `/me/sop/tasks`**，看有没有 `running` / `pending` 的任务。
...
**登录方式**：Web 端不走 `wx.login()`，见 §3.8.6。

#### 3.8.5 Web 端登录：由你们后端代换 token

Web 端没有 `wx.login()` 的 code，走另一条路：**你们后端验完用户身份，替他向服务端换一个会话 token 给前端用**。

```
① Web 前端 ──登录──► 你们的后端
                        │ 用 ak+sk（保存在后端）走 OAuth2 拿到 JWT（含 user_id）
                        │
                        ├──② POST /auth/web-login ──► 熵玑参谋服务端
                        │     Header: X-Admin-Token: <管理员令牌>
                        │     Body:   {"user_id": "<JWT 里的用户标识>"}
                        │
                        ◄──③ {token, openid, is_new_user, expires_at}
                        │
③ Web 前端 ◄── 你们的 JWT + 服务端的 session token ──┘
                        │
④ Web 前端 ──Bearer <session token>──► /chat、/events、/me/*
```

**服务端不解析你们的 JWT**——那是你们的凭据体系，我们不碰。你们验完身份后把 `user_id` 报过来即可。

##### `POST /auth/web-login`

**鉴权：`X-Admin-Token`**（管理员令牌，运维提供）。这是服务端到服务端接口。

请求体：

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `user_id` | string | **是** | 你们体系里的用户标识，通常取 JWT 的 `sub`。空字符串返回 400 |
| `display_name` | string | 否 | 昵称 |
| `avatar_url` | string | 否 | 头像 URL |
| `breed` | string | 否 | 把这个用户开在哪个**品种**上。不传 = 服务端默认品种 |

响应体与 §3.1 完全相同（`token` / `openid` / `is_new_user` / `expires_at`）。
`openid` 为 `web:<user_id>` 形式。

###### 关于 `breed`

一个「品种」= 一整套龙虾（人格 + 技能 + SOP）。同一台服务器可以同时跑多种，
`breed` 决定这个用户对话的是哪一只。

- **不传就是默认品种**，老代码不用改，行为与以前一字不差
- **只在创建用户那一次生效**。已存在的用户带上不同的 `breed` 再登录，**品种不会变**
  ——换品种会重启对方的龙虾并把旧技能从工作区删掉，不该由一次普通登录顺带触发。
  确实要换，走运维接口 `PUT /admin/users/<openid>/breed`
- **品种名要先在服务器上存在**，传一个不存在的名字会返回错误（而且不会创建用户）。
  当前可用的品种向运维确认
- **为什么这个接口可以由你们指定品种，而小程序的 `/auth/wx-login` 不行**：这个接口
  有 `X-Admin-Token`，调用方是你们后端，可信；`/auth/wx-login` 是公网设备侧接口、
  没有鉴权，让客户端指定品种等于让用户自选人格和技能权限。**同一个需求，两个接口
  的正确答案是相反的**

##### ⚠️ 三件必须注意的事

1. **`user_id` 必须长期稳定。** 它是这个用户工作区、对话记忆、历史简报的主键。换了 `user_id` 等于换了个人——旧的记忆和简报全都看不到了。别用会变的值（如手机号、邮箱）。

2. **管理员令牌绝不能下发到前端。** 它权限很大（可创建用户、查询全部用户列表），只能保存在你们后端。前端只拿换回来的 session token。

3. **两端各自独立（已确定的产品决策）。** Web 用 `web:<user_id>`，小程序用微信 `openid`，是两个不同的键——同一个人从两端登录会得到**两套互不相通的对话记忆和简报库**。这是有意为之，不是缺陷，双方按此实现即可，无需做身份映射。

   **但前端需要照顾用户预期**：用户不会知道背后是两套账号，在小程序问过的公司、跑过的简报，到 Web 端一条都看不到，很容易被当成"数据丢了"来报障。建议在 Web 端首次进入时给一句轻提示（如"网页版与小程序的记录相互独立"），或在历史为空时的空态里说明，比事后解释省事。
...
每个用户有独立的每日消耗上限，**超出后 `/chat` 返回 400**，响应体：

```json
{"error": "今日额度已用完，请稍后再试或联系管理员调整额度上限。"}
```

**前端要点**：

- 这是**正常的业务状态，不是故障** —— 请原样把文案展示给用户，别弹「系统错误」
- 额度**按北京时间**每日零点重置（不是 UTC）
- 一次企业快评的消耗远大于普通问答，联调时连续跑几次就可能触顶

当前测试期上限约合每用户每天 10 次企业快评。正式运营前会按真实使用情况调整，届时以运维通知为准。
...
- 企业快评："查一下 XX 公司""XX 这家公司怎么样""帮我看看 XX""评估 XX"
  - **支持简称**，不必给工商全称。说"查一下商汤科技""查一下地平线"都能识别，流程第一步会自行消歧到唯一的注册主体
